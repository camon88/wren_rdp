...
public class WrenRDP extends RDP implements WrenTokens {
    ...
    private void program() {
        match(PROG_TOK);
        debug(" PROG_TOK matched in program");
        match(VARIABLE_TOK);
        debug(" VARIABLE_TOK matched in program");
        match(IS_TOK);
        debug(" IS_TOK matched in program");
    ...
    private void dec() {
        match(VAR_TOK);
        debug(" VAR_TOK matched in dec");
        debug("  varlist called in dec");
        varlist();
        match(COLON_TOK);
        debug(" COLON_TOK matched in dec");
        debug("  type called in dec");
        type();
        match(SEMICOLON_TOK);
        debug(" SEMICOLON_TOK matched in dec");
    }

    private void type() {
        if (currTok == INT_TOK) {
            match(INT_TOK);
            debug(" march INT_TOK in type");
        } else if (currTok == BOOL_TOK) {
            match(BOOL_TOK);
            debug(" match BOOL_TOK in type");
        } else error("type");
    }
    private void varlist() {
        if (currTok == VARIABLE_TOK) {
            match(VARIABLE_TOK);
            debug(" VARIABLE_TOK matched in varlist");
            if (currTok == COMMA_TOK) {
                match(COMMA_TOK);
                debug(" COMMA_TOK matched in varlise");
    ...
    private void varlist2() {
        if (currTok == VARIABLE_TOK) {
            match(VARIABLE_TOK);
            debug(" VARIABLE_TOK matched in varlist2");
            match(COMMA_TOK);
            debug(" COMMA_TOK matched in varlist2");
    ...
    private void commandseq() {
        ...
        if (currTok == SEMICOLON_TOK) {
            ...
            match(SEMICOLON_TOK);
            debug(" SEMIOCLON_TOK matched in commandseq");
    ...
    private void commandseq2() {
        if (currTok == SEMICOLON_TOK) {
            match(SEMICOLON_TOK);
            debug(" SEMICOLON_TOK matched in commandseq2");
            commandseq2();
            command();
        }
    ...
    private void command() {
        if (currTok == VARIABLE_TOK) {
            ...
            match(SKIP_TOK);
            debug(" SKIP_TOK matched in command");
        } else if (currTok == READ_TOK) {
            match(READ_TOK);
            debug(" match READ_TOK in command");
            match(VARIABLE_TOK);
            debug(" match VAR_TOK in command");
        } else if (currTok == WRITE_TOK) {
            match(WRITE_TOK);
            debug(" match WRITE_TOK in command");
            debug(" call intexpr from command");
            intexpr();
        } else if (currTok == WHILE_TOK) {
            match(WHILE_TOK);
            debug(" WHILE_TOK match in command");
            debug(" call boolexpr from command");
            boolexpr();
            match(DO_TOK);
            debug(" DO_TOK matched in command");
            commandseq();
            if (currTok == END_TOK) {
                match(END_TOK);
                debug(" END_TOK matched in command");
                match(WHILE_TOK);
                debug(" WHILE_TOK matched in command");
            } else if (currTok != END_TOK) {
                commandseq();
                match(END_TOK);
                debug(" END_TOK matched in while loop in command");
                match(WHILE_TOK);
                debug(" WHILE_TOK matched at end of while loop in command");
            }
        } else if (currTok == IF_TOK) {
            match(IF_TOK);
            debug(" IF_TOK matched in command");
            debug(" call boolexpr from command");
            boolexpr();
            match(THEN_TOK);
            debug(" THEN_TOK matched in command");
    ...
    private void strong_op() {
        if (currTok == MUL_TOK) {
            match(MUL_TOK);
            debug(" MUL_TOK matchedi in strong op");
        } else if (currTok == DIV_TOK) {
            match(DIV_TOK);
            debug(" DIV_TOK matched in strong op");
        } else error("strong op");
    }
    private void weak_op() {
        if (currTok == PLUS_TOK) {
            match(PLUS_TOK);
            debug(" PLUS_TOK matched in weak op");
        } else if (currTok == MINUS_TOK) {
            match(MINUS_TOK);
            debug(" MINUS_TOK matched in weak op");
        } else error("weak op");
    }
    private void intelement() {
        if (currTok == INTCONST_TOK) {
            match(INTCONST_TOK);
            debug("INTCONST_TOK matched in intelement");
        } else if (currTok == VARIABLE_TOK) {
            match(VARIABLE_TOK);
            debug("VARIABLE_TOK matched in intelement");
            ...
            intexpr();
            match(RPAR_TOK);
            debug("RPAR_TOK matched in intelement");
        } else if (currTok == MINUS_TOK) {

            match(MINUS_TOK);
            debug("MINUS_TOK matched in intelement");
    ...
    private void boolexpr2() {
        if (currTok == OR_TOK || currTok == FALSE_TOK || currTok == NOT_TOK) {
            match(OR_TOK);
            debug("OR_TOK matched in boolexpr2");
    ...
    private void boolterm2() {
        if (currTok == OR_TOK || currTok == AND_TOK || currTok ==
            INTCONST_TOK) {
            match(currTok);
            debug(Integer.toString(currTok));
    ...
    private void boolelement() {
        ...
        } else if (currTok == INTCONST_TOK || currTok == LPAR_TOK ||
            currTok == MINUS_TOK) {
            intexpr();
            relation();
            intexpr();
